...
- Keep `data/dataset_v2_clean` unchanged.
- Collect recovery-only demonstrations in `data/dataset_v3_recovery`.
...
caffeinate -dimsu env \
  ...
  python scripts/collect_data_v3.py \
    --num-episodes 600 \
    --data-dir data/dataset_v3_recovery \
    --seed 20261201 \
  2>&1 | tee results/training/v3/collection_console_v3.log
```
...
conda run --no-capture-output -n mujoco310 \
  python scripts/preflight_v3.py \
    --base-data-dir data/dataset_v2_clean \
    --recovery-data-dir data/dataset_v3_recovery \
    --expected-base-episodes 1200 \
    --expected-recovery-episodes 600 \
    --init-policy artifacts/v2-clean-rc1/mini_vla_v2_clean_policy.pth \
    --batch-size 32 \
    --num-workers 4 \
    --local-files-only \
    --output results/training/v3/preflight_report_v3.json
```
...
conda run --no-capture-output -n mujoco310 \
  python scripts/evaluate_policy_v2.py \
    ...
    --render \
    --local-files-only \
    --output-prefix results/benchmarks/language/eval_pick_A_heldout
```
