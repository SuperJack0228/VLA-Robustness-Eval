...
environment installed. Large datasets under `data/` and generated runs under
`results/` are intentionally excluded from Git. Frozen policies and provenance
...
```bash
mkdir -p results/training/v2_clean
set -o pipefail
...
caffeinate -dimsu env \
  ...
  python scripts/collect_data_v2.py \
    --num-episodes 1200 \
    --data-dir data/dataset_v2_clean \
    --seed 20260817 \
  2>&1 | tee results/training/v2_clean/collection_console_v2_clean.log
```
...
caffeinate -dimsu env \
  ...
  python scripts/train_v2.py \
    --data-dir data/dataset_v2_clean \
    --output-dir results/training/v2_clean \
    --epochs 40 \
    --batch-size 32 \
    --num-workers 8 \
    --init-policy artifacts/v2-clean-rc1/warm_start_v2_2_policy.pth \
    --init-scope perception \
    --local-files-only \
  2>&1 | tee results/training/v2_clean/training_console_v2_clean.log
```
...
caffeinate -dimsu env \
  ...
  python scripts/train_v2.py \
    ...
    --batch-size 32 \
    --num-workers 8 \
    --resume results/training/v2_clean/mini_vla_v2_clean_last.pth \
    --local-files-only \
  2>&1 | tee -a results/training/v2_clean/training_console_v2_clean.log
```
...
python scripts/postflight_v2.py \
  --policy artifacts/v2-clean-rc1/mini_vla_v2_clean_policy.pth \
  --data-dir data/dataset_v2_clean \
  --output results/training/v2_clean/postflight_v2_clean.json \
  --batch-size 32 \
  --num-workers 4 \
  --local-files-only \
  --enforce
...
python scripts/evaluate_policy_v2.py \
  ...
  --log-every 20 \
  --output-prefix results/training/v2_clean/evaluation_clean_v2_clean \
  --local-files-only \
  --enforce-80 \
  2>&1 | tee results/training/v2_clean/evaluation_clean_v2_clean.log
```
